...
- Risk stratification by patient and utilization groups
...
- Created reproducible Python/Jupyter analysis outputs for Table 1 baseline characteristics, readmission summaries, outpatient follow-up summaries, ED revisit summaries, risk stratification, and exploratory logistic regression.
...
I built a synthetic EHR post-discharge utilization analytics project to demonstrate the workflow behind retrospective healthcare analytics requests. Using Synthea data, SQL, Python, and Jupyter, the project defines an adult inpatient cohort, derives outpatient follow-up timing, ED revisit, and 30-day readmission measures, performs validation QA, creates dashboard-ready aggregate tables, summarizes risk strata, and fits an exploratory logistic regression model. The repository emphasizes reproducibility, data governance, cohort logic, temporal validation, analytic interpretation, and healthcare analytics communication without using real patient data.
